...
function Catatan() {
  ...
  const updateCatatan = () => {
    const updatedNotes = notes.map((note) =>
      note.id === editId ? { ...note, judul, isi } : note
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-pink-100 to-white p-10">
      {/* Judul */}
      ...
      <div className="max-w-xl mx-auto bg-white p-6 rounded-2xl shadow-lg mb-12">
        ...
        <button
          onClick={editId ? updateCatatan : tambahCatatan}
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {notes.map((note) => (
          ...
          >
            ...
            <p className="text-gray-600 text-sm mb-4">{note.isi}</p>

            <div className="flex gap-4">
              <button
                onClick={() => editCatatan(note)}
